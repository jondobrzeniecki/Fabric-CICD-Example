...
%pip install imblearn imbalanced-learn==0.12.4
...
df['Air_temperature_K'] = df['Air_temperature_K'].astype(float)
df['Process_temperature_K'] = df['Process_temperature_K'].astype(float)
...
columns = ['Air_temperature_K', 'Process_temperature_K', 'Rotational_speed_rpm', 'Torque_Nm', 'Tool_wear_min']
data=df.copy()
...
features = df[['Type', 'Air_temperature_K', 'Process_temperature_K', 'Rotational_speed_rpm', 'Torque_Nm', 'Tool_wear_min']]
labels = df['IsFail']
...
# MAGIC from sklearn.metrics import f1_score, accuracy_score, recall_score
# MAGIC 
# MAGIC mlflow.set_experiment("Machine_Failure_Classification")
# MAGIC mlflow.autolog(exclusive=False) # Override the pre-configured autologging behavior
# MAGIC 
# MAGIC with mlflow.start_run() as run:
# MAGIC     rfc_id = run.info.run_id
# MAGIC     print(f"run_id {rfc_id}, status: {run.info.status}")
# MAGIC     rfc = RandomForestClassifier(max_depth=5, n_estimators=50)
# MAGIC     rfc.fit(X_train_res, y_train_res) 
# MAGIC     signature = infer_signature(X_train_res, y_train_res)
# MAGIC 
# MAGIC     mlflow.sklearn.log_model(
# MAGIC         rfc,
# MAGIC         "machine_failure_model_rf",
# MAGIC         signature=signature,
# MAGIC         registered_model_name="machine_failure_model_rf"
# MAGIC     ) 
# MAGIC 
# MAGIC     y_pred_train = rfc.predict(X_train)
# MAGIC     # Calculate the classification metrics for test data
# MAGIC     f1_train = f1_score(y_train, y_pred_train, average='weighted')
# MAGIC     accuracy_train = accuracy_score(y_train, y_pred_train)
# MAGIC     recall_train = recall_score(y_train, y_pred_train, average='weighted')
# MAGIC 
# MAGIC     # Log the classification metrics to MLflow
# MAGIC     mlflow.log_metric("f1_score_train", f1_train)
# MAGIC     mlflow.log_metric("accuracy_train", accuracy_train)
# MAGIC     mlflow.log_metric("recall_train", recall_train)
# MAGIC 
# MAGIC     # Print the run ID and the classification metrics
# MAGIC     print("F1 score_train:", f1_train)
# MAGIC     print("Accuracy_train:", accuracy_train)
# MAGIC     print("Recall_train:", recall_train)    
# MAGIC 
# MAGIC     y_pred_test = rfc.predict(X_test)
# MAGIC     # Calculate the classification metrics for test data
# MAGIC     f1_test = f1_score(y_test, y_pred_test, average='weighted')
# MAGIC     accuracy_test = accuracy_score(y_test, y_pred_test)
# MAGIC     recall_test = recall_score(y_test, y_pred_test, average='weighted')
# MAGIC 
# MAGIC     # Log the classification metrics to MLflow
# MAGIC     mlflow.log_metric("f1_score_test", f1_test)
# MAGIC     mlflow.log_metric("accuracy_test", accuracy_test)
# MAGIC     mlflow.log_metric("recall_test", recall_test)
# MAGIC 
# MAGIC     # Print the classification metrics
# MAGIC     print("F1 score_test:", f1_test)
# MAGIC     print("Accuracy_test:", accuracy_test)
# MAGIC     print("Recall_test:", recall_test)
